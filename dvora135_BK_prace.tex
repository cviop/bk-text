...
Tato kapitola se zabývá samotnou realizací sondy. Popisuje cestu, kterou byla práce směřována při vývoji elektroniky a popisuje výběr jednotlivých komponent. Dále vysvětluje dílčí části firmwaru sondy a způsob, jakým byly řešeny problémy. Popisuje jak vysílací část - sondu, tak přijímací části. Jednak pozemní stanici, která formátovala data do zprávy čitelné anténním trackerem a jednak přijímací software na zobrazení pozice na mapě.
	...
	Délka zprávy odeslané GPS přijímačem se liší podle dostupných údajů. Pokud GPS nezná svou polohu, pole pro zeměpisnou šířku a délku jsou prázdná. Proto nemůžeme příjimat zprávu fixní velikost. Způsob, kterým se ve firmwaru sondy NMEA zpráva přijímá je pomocí následující funkce.
	\lstinputlisting[language=C]{Code/HAL_UARTEx_ReceiveToIdle_IT.c}

	Tato funkce přijímá data až do doby, kdy nenastane klidový stav - GPS přestane vysílat data. V ten moment se vyvolá callback funkce zanesená 
	\lstinputlisting[language=C,caption={Callback funkce vyvolaná interruptem}, label={lst:main:callback}]{Code/callback.c}
	V této funkci se do globální proměnné DataRecieved uloží velikost přijatých dat. Pokud je v hlavní smyčce programu splněna podmínka, že DataRecieved > 0, program začne rozebírání přijatých dat funkcí níže. Pro rozdělení přijatých dat na jednotlivé NMEA zprávy se využívá znaků <CR> <LF>, které se nacházejí na konci každého řádku. Součástí vyslaných dat jsou i \$GPGSV zprávy obsahující informace o viditelných satelitech. Zpráv tohoto typu je vysílá několik po sobě a počet vyslaných zpráv je zanesen v prvním poli této zprávy.

	\lstinputlisting[language=C,caption={Hlavní parsovací funkce}, label={lst:main:parse}]{Code/main_parse.c}

	Funkce uloží data do bufferu \lstinline{uint8_t GPSparse[15][200] = }  tvořeného 2D polem a zjistí počet \$GPGSV zpráv.

	Volání funkce v hlavní smyčce programu vypadá následovně.
	\lstinputlisting[language=C]{Code/parse_in_main.c}
...
% Note: It may be necessary to compile the document several times to get a multi-page table to line up properly
	...
	Identifikátor rozd. reference &            &         &                             \\ \hline
	Checksum                      & *5D        &         &                             \\ \hline
	<CR> <LF>                     &            &         & Konec zprávy                \\ \hline
	\end{longtable}
