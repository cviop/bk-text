...
\chapter{Výsledky}
	...
	\begin{align}
		...
		\label{eq:refr:meas}
	\end{align}
	kde $T$ je absolutní hodnota v K, $p$ je atmosférický tlak v hPa a $e$ je tlak vodních par v hPa. Ten lze vypočíst pomocí hustoty vodních par z doporučení ITU-R 836. Pdle [zdroj 5] lze tlak vodních par vypočítat také podle relativní vlhkosti vzduchu $H$ jako
	\begin{align}
	...
	\end{align}
	kde $h$ je výška nad zemí, $N_\mt{0}$ hodnota refraktivity v 0~km a $h_\mt{0}$ výškové měřítko v km. ITU udává globální světový průměr $N_\mt{0} = 315 \mt{ N}$ a $h_\text{0} = 7{,}35 \mt{ km}$.
	...
	Data refraktivity vypočtené pomocí \eqref{eq:refr:meas} byla proložena exponenciální funkcí. Výpočet koeficientů byl proveden pomocí funkce v Matlabu \lstinline[language=Matlab]|fit(alt,N,'exp1');|, kde \lstinline|alt| je vektor nadmořských výšek v metrech a \lstinline|N| je vektor vypočtených refraktivit pro danou nadmořskou výšku.
